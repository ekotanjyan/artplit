 ...
 * Plugin URI: https://zotabox.com/zbv2/contactform?utm_source=wordpress.com&utm_medium=Contact%20Form&utm_campaign=ecommerce%20plugins&authuser=anonymous
 * Description: Fully Customizable and Responsive Contact Form. Add fields and background image. 20 more popular marketing tools also incl.
 * Version: 2.1.6
 * Author: Zotabox
 * Author URI: https://zotabox.com/dashboard/?utm_source=wordpress.com&utm_medium=Contact%20Form&utm_campaign=ecommerce%20plugins&authuser=anonymous
 * License: SMB 1.0
 */

//Fix some small bugs

add_action( 'admin_init', 'zb_cf_admin_init' );
function zb_cf_admin_init(){
	/* Register stylesheet. */
...
register_deactivation_hook( __FILE__, 'zb_cf_deactivate' );
function zb_cf_deactivate(){
	update_option( 'ztb_status_message', 2 );
	update_option( 'ztb_status_disconnect', 1 );
}

register_activation_hook( __FILE__, 'zb_cf_activate' );
function zb_cf_activate() {
	update_option( 'ztb_status_message', 1 );
}

/*
add_action('admin_notices', 'zb_cf_show_admin_messages');
function zb_cf_show_admin_messages() {

}
*/

add_filter( 'plugin_action_links_' . plugin_basename(__FILE__), 'zb_cf_add_action_links' );
function zb_cf_add_action_links ( $links ) {
	 $mylinks = array(
	 '<a href="' . admin_url( 'admin.php?page=zb_cf' ) . '">Settings</a>',
	 );
	return array_merge( $links, $mylinks );
}

add_action('admin_menu', 'zb_cf_admin_menu');
function zb_cf_admin_menu() {
	add_menu_page('Better Contact Form', 'Contact Form', 'administrator', 'zb_cf', 'zb_cf_setting',plugins_url( 'bcf_icon.png', __FILE__ ));
}

function zb_cf_setting(){
	...
	if(isset($access_key) && !empty($access_key) && strlen($access_key) > 0 && $ztb_status_disconnect == $connected){
		...
		$button = '<a  target="zotabox" href="'.$domain_action.'/customer/access/PluginLogin/?customer='.$ztb_id.'&access='.$access_key.'&app=cf&platform=wordpress&utm_source=wordpress.com">
			Configure your tools
		</a>';
		$form = '';
	}else{
		$form = '<form class="ztb-register-form" target="_blank" method="POST" action="'.$domain_action.'/customer/access/PluginAuth?app=cf&utm_source=wordpress.com&utm_medium=Contact%20Form&utm_campaign=ecommerce%20plugins&platform=wordpress&access='.$access_key.'" id="account-info">
	...
	<div class="ztb-wrapper">
		<div class="ztb-logo">
			<a href="https://zotabox.com/dashboard/?utm_source=wordpress.com&utm_medium=Contact%20Form&utm_campaign=ecommerce%20plugins&authuser=anonymous" title="Zotabox" target="zotabox">
				<img title="Zotabox" alt="Zotabox" src="'.plugins_url( 'assets/images/logo-zotabox.png', __FILE__ ).'">
			</a>
		</div>
		<div class="ztb-code-wrapper wrap">
			<div class="ztb-title">
				Fully Customizable and Responsive Contact Form. Add fields and background image. 10+ more popular marketing tools also included.
			</div>
...
function insert_zb_cf_code(){
	if(!is_admin()){
		...
		if(!empty($domain) && strlen($domain) > 0 && $ztb_status_disconnect == $connected){
			print_r(html_entity_decode(print_zb_cf_code($domain)));
		}
	}
}

add_action( 'wp_head', 'insert_zb_cf_code' );


add_action("wp_ajax_update_zb_cf_code", "update_zb_cf_code");
add_action("wp_ajax_nopriv_update_zb_cf_code", "update_zb_cf_code");

function update_zb_cf_code(){
	...
	if(!isset($domain) || empty($domain)){
		header("Location: ".admin_url()."admin.php?page=zb_cf");
	}else{
...
function print_zb_cf_code($domainSecureID = "", $isHtml = false) {
